...
describe(`getExpectedComponentName`, () => {
  it('strips .mina', () => {
    ...
    expect(getExpectedComponentName('../myComp/nameAndId.mina')).toEqual('name-and-id');
  });

  it('does some specific handling with scoped packages', () => {
    expect(getExpectedComponentName('~@irbnb/some-package-name/comp.mina')).toEqual('name-comp');
    expect(getExpectedComponentName('~@irbnb/some-package-name/comp/index.mina')).toEqual(
      'name-comp',
    );
    expect(getExpectedComponentName('~@irbnb/some-package-name/myComp/index.mina')).toEqual(
      'name-my-comp',
    );
  });
});
